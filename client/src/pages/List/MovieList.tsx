    ...
    const fetchListData = async () => {
      if (user?.uid && listId) {
        try {
          ...
          const list = userData.lists.find((l: List) => l.list_id === listId);
          if (list) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">{listName || "My Movie List"}</h1>
        <Dialog open={isSearchOpen} onOpenChange={setIsSearchOpen}>
          ...
          <DialogContent>
            <h2 className="text-lg font-semibold mb-4">Add a Movie to Your List</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
        ...
        <Card className="overflow-hidden border-dashed">
          <CardContent className="p-0 flex items-center justify-center h-full aspect-[2/3]">
            <div className="text-center p-4" onClick={() => setIsSearchOpen(true)}>
              <Plus className="mx-auto h-8 w-8 text-muted-foreground mb-2" />
              <p className="text-sm text-muted-foreground">Add Movie</p>
            </div>
          </CardContent>
        </Card>
      </div>
