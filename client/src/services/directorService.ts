...
const fetchPerson = async (searchTerm: string, department: 'Directing' | 'Acting') => {
  if (searchTerm.length < 3) return null;
  
  const url = `https://api.themoviedb.org/3/search/person?query=${encodeURIComponent(searchTerm)}&include_adult=false&language=en-US&page=1`;
  ...
  const filteredResults = data.results.filter((person: any) => person.known_for_department === department);
  console.log(data.results);
  
  return { ...data, results: filteredResults };
};

export const useSearchPerson = (searchTerm: string, department: 'Directing' | 'Acting') => {
  return useQuery({
