...
router.post('/:listId/add-movie', verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    // Find the list
    const list = await ListModel.findOne().where('list_id').equals(listId);
    ...
    const isMember = list.members.some(member => member.user_id === userId);
    if (!isMember) {
      return res.status(403).json({ error: 'You are not a member of this list' });
    }

    // Check if the movie already exists in the list
    const movieExists = list.movies.some(movie => movie.movie_id === movie_id);
    if (movieExists) {
      return res.status(400).json({ error: 'Movie already exists in the list' });
    }

    // Add the movie to the list
    list.movies.push({ movie_id, title, poster_path, release_date });
    await list.save();
